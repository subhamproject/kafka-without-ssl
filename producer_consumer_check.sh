...
./kafka-console-producer.sh --broker-list kafka3:9092 --topic test    

./kafka-console-consumer.sh --bootstrap-server kafka1:9092 --topic test --from-beginning  

echo "Hello World, this sample provided by ATA" | ./kafka-console-producer.sh --broker-list kafka1:9092 --topic ATA > /dev/null

./kafka-console-consumer.sh --bootstrap-server kafka3:9092 --topic ATA --from-beginning
